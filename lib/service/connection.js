...
class Connection extends EventEmitter {
  constructor(config) {
    ...
    this.imap.on('mail', () => this.checkNewMails());
  ...
  error(err) {
    ...
    this.box = null;
    this.mailChecker && clearInterval(this.mailChecker);
  ...
  endOpenBox(err, box) {
    ...
    this.imap.search([ ['SINCE', since] ], (err, results) => {
      ...
      this.fetchMessages(results, () => {
        this.mailChecker = setInterval(() => this.checkNewMails(), 30000);
      });
    });
  }

  fetchMessages(msgs, done) {
    logger.debug('fetchMessages', msgs);
    ...
    fetch.on('end', () => { done && done(); this.emit('fetchEnd'); });
    this.emit('fetchBegin');
  }

  checkNewMails() {
    if(!this.box) { return; }
    this.fetchMessages(`${this.lastSeqno+1}:*`);
  }

  readMessage(msg, seqno) {
    logger.debug('readMessage', seqno);
    if(!this.lastSeqno || this.lastSeqno < seqno) {
    ...
    const dispatchMessageIfReady = () => {
      if(ended && streamCount === 0) {
        logger.debug('dispatchMessage', seqno);
