...
class Database extends EventEmitter {
  ...
  addMessage(html, id) {
    ...
    logger.debug(`parsing ${rows.length} rows from message ${id}`);
  ...
  _addRow(row) {
    try {
      ...
      console.log(parts[2], new Date(date).toString());
  ...
  _setState(state) {
    logger.debug(`setState ${state.zone}, ${new Date(state.date).toString()}, ${state.active}`);
    const { zone } = state;
